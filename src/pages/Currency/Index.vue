<template>
  <q-page>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:top>
            ...
            <add-button
              v-if="Utils.hasPermissions(['Currency: Create Currency'])"
              :disable="loading"
              @click="showAdd = true"
              tooltip-text="Add"
              color="primary"
          ...
          <template v-slot:body-cell-actions="props">
            <q-td class="text-center">
              <q-btn
                v-if="Utils.hasPermissions(['Currency: Edit/Update Currency'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
              ...
              <q-btn
                v-if="Utils.hasPermissions(['Currency: Delete Currency'])"
                class="q-mr-sm"
                size="xs"
                rounded
                padding="5px"
    ...
    <q-dialog v-model="showAdd" position="top" persistent>
      <add-currency @onClose="showAdd = false" @onAdded="onRefresh" />
    </q-dialog>
...
import AddCurrency from "../../components/Currency/Add.vue";
import EditCurrency from "../../components/Currency/Edit.vue";
import Confirm from "../../components/Shared/Confirm.vue";

const { loading, columns, items, trash, paginate } = useCurrency();
const {
  showAdd,
  showEdit,
  selected,
